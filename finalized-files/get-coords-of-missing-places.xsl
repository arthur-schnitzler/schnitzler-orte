...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="place"
        select="document('missing-places.xml')"/>
        ...
        <xsl:element name="location" namespace="http://www.tei-c.org/ns/1.0">
            <xsl:element name="geo" namespace="http://www.tei-c.org/ns/1.0">
                <xsl:attribute name="lat">
                    <xsl:copy-of select="key('place-lookup', $ortsname, $place)//tei:geo[@ana='lat']"/>
                </xsl:attribute>
                <xsl:attribute name="long">
                    <xsl:copy-of select="key('place-lookup', $ortsname, $place)//tei:geo[@ana='long']"/>
                </xsl:attribute>
            </xsl:element> 
        </xsl:element>
    </xsl:template>
